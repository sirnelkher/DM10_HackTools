/*************************************************************************************************************
 * Alesis DM10 - Decode Syx File to Rom File
 *
 * Author: Fabrício Dias, fmgdias@gmail.com
 *
 * Compile:
 *      cd /Users/fmgdias/Dropbox/Desenvolvimento/DM10/DM10_HackTools/DM10_syx2rom/ ;
 *      gcc syx2rom.c -o syx2rom && ./syx2rom ../../syx/sound/DM10_sound_rom-V1.03 ;
 *************************************************************************************************************/
...
int main(int argc, char **argv)
{
    ...
    snprintf(fDestinyName, 256, "%s.rom", argv[1]);
    ...
    while (!feof(fSourceHandler))
    {
        ...
        if ( fChr == 0xF7 )
        {
            if ( fSourceBuffer[0] == 0xF0 && fSourceBuffer[1] == 0x00 && fSourceBuffer[2] == 0x00 && fSourceBuffer[3] == 0x0E && fSourceBuffer[5] == 0x03 )
            {
                ...
                //Skip last 11 bytes
                romPayloadLen -= 11;
                //Write, Skip last 11 bytes. Data (192 bytes) + Address (8 bytes) + Control (2 bytes) +Checksum (1 byte)
                for (i = 0; i < romPayloadLen; i++)
                {
                ...
                {
                    printf("\n > Processed: %d KBytes", (fDestinyCount * 192 / 1024));
                }
            }
            //Clean Buffer
